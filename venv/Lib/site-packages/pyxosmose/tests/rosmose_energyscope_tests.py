...
from pyxosmose.tests.rosmose_es_utility import *


class RosmoseEnergyscopeTests(unittest.TestCase):
    ...
    def test_es_creation(self):
        res = self.ro.retrieve_type(create_es_model())
        self.assertTrue(res[0])
        soft = self.ro.software_manager.get_model_by_name("myESModel")
        self.assertEqual(soft["name"], "myESModel")
        self.assertEqual(soft["software"], "ENERGYSCOPE")
        self.assertEqual(soft["comment"], "")
        self.assertTrue(len(soft["inputs"]) == 0)
        self.assertTrue(len(soft["outputs"]) == 0)

    # def test_es_software_inputs_and_outputs(self):
    #     res = self.ro.retrieve_type(create_es_model())
    #     self.assertTrue(res[0])
    #     res = self.ro.retrieve_type(create_es_inputs())
